...
╔════════════════════════════════════════════════════╗
║ Z O R A  ::  A U T O  S W A P  :: 0 x E N I G M A  ║
║ MODE : AUTO BUY TARGET USERNAME                    ║
║ STATUS : ACTIVE                                    ║
╚════════════════════════════════════════════════════╝
{C.RESET}""")
...
GRAPHQL_BODY = {
    ...
    "variables": {"first": 24, "listType": "NEW_CREATORS"},
...
def get_creators():
    try:
        ...
        edges = r.json()["data"]["exploreList"]["edges"]
        return [e["node"] for e in edges]
    except Exception as e:
        log_error(e)
        return []

def find_target_contract():
    for node in get_creators():
        ...
        log_scan(f"Found handle → {handle}")

        if handle == TARGET_USERNAME:
            addr = node.get("address")
            if addr:
                log_success(f"TARGET MATCH → {handle} | {addr}")
...
async def main():
    wallets()
    log_info(f"Watching username → {TARGET_USERNAME}")

    seen = set()

    while True:
        try:
            contract = find_target_contract()

            if contract and contract not in seen:
                seen.add(contract)

                log_buy(f"AUTO SWAP → {contract}")
                tx = buy_token(contract)
                log_success(f"TX SENT → {tx}")
